...
\usebeamerfont{sansserif}

% Remove navigation symbols
\setbeamertemplate{navigation symbols}{}

% Set frame title font to a blocky (sans-serif) style
...
\newcommand{\crossoutframe}{%
    \begin{tikzpicture}[remember picture, overlay]
        \draw[red, line width=3pt, shorten >=2cm, shorten <=2cm]
...
\begin{frame}[label=overview]{My talk today}
    \begin{enumerate}
        ...
        \item \textbf{Compositionality and structural representations.} A candidate for structural representation and semantic information
        \item \textbf{Proposal.} Noisy addition as a pathway to linearity in multidimensional spaces
...
\begin{frame}[label=gap] {(Some) kinds of information (after 1948)}
        ...
        \begin{tikzpicture}
            ...
            \node[fill=gray!20, ellipse, inner sep=3pt] at (-2, -2.5) {Strongly semantic information};

            \node[fill=yellow!20, ellipse, inner sep=3pt] at (-2, -1) {Natural information};
            \node[fill=red!20, ellipse, inner sep=3pt] at (3, -2) {Non-natural information};

            \node[fill=purple!20, ellipse, inner sep=3pt] at (-3, -4.5) {Biological information};
            \node[fill=orange!20, ellipse, inner sep=3pt] at (3, -4) {Mental representation};
            \node[fill=cyan!20, ellipse, inner sep=3pt] at (-1, -4) {Neural information};
...
\begin{frame}[label=info_kinds]{Kinds of Information Explained}
    \begin{itemize}
        \item \textbf{Shannon Information:} Measures the variability or uncertainty in a collection of objects, often used in communication theory to quantify the amount of information transmitted.
        \item \textbf{Fisher Information:} Represents the amount of information that an observable random variable carries about an unknown parameter, commonly used in statistics and parameter estimation.
        \item \textbf{Semantic Information:} Information that has meaning or "aboutness," often tied to truth conditions and used in contexts like language and representation.
        \item \textbf{Strongly Semantic Information:} A stricter form of semantic information that is truth-entailing and cannot misrepresent.
        \item \textbf{Natural Information:} Information that occurs naturally in the environment, such as the correlation between smoke and fire.
        \item \textbf{Non-Natural Information:} Information that is intentionally created or conveyed, such as linguistic statements or symbols.
        \item \textbf{Biological Information:} Information encoded in biological systems, such as DNA sequences or neural signals.
        \item \textbf{Mental Representation:} Information processed or stored in the mind, often tied to cognitive states or intentionality.
        \item \textbf{Neural Information:} Information encoded in the activity of neurons, such as spike trains or patterns of neural firing.
        \item \textbf{Functional Information:} Information that is tied to the function or purpose of a system, often used in engineering or systems biology.
        \item \textbf{Non-Semantic Information:} Information that does not carry meaning or intentionality, often used in purely physical or mechanical contexts.
    \end{itemize}
\end{frame}
...
\begin{frame}[label=carnap_bar_hillel]{Probably first mention about semantic information}
   
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.6\textwidth]{images/Carnap_1952.png}
    \caption{Four years after Shannon introducing the entropy of information new term is coined by Carnap and}
    \label{fig:pimage}
\end{figure}
\end{frame}
...
\begin{frame}[label=basic_topologies]{Basic multidimensional pipelines}
...
\node[left=0.5cm of a2] {many-to-many-straight};
...
\node[left=0.5cm of c0] {one-to-many, more efficient };
...
\node[left=0.5cm of d2] {many-to-one, more efficient};
...
\begin{frame}[label=semantic_meaning] {What does "semantic" mean, roughly}
    \begin{center}
        Semanticity is a property of representation sytems (words, images, maps) that are about something, that have "aboutness" or "intentionality" (in the sense of Brentano, \cite{brentano_psychology_1995}) - but in a distinguished way: systematic way.
        ...
        \begin{itemize}
            \item Terms that refer to the domain, such as logic constants (e.g., "and", "or", "not") and quantifiers.
            \item Recursive rules for constructing properly built compound representations.
            \item Ensuring that what the compound representations refer to is a function of the referential terms and logical constants.
        \end{itemize}
...
\begin{frame}{Taking collection for its measure }
   ...
   We usually do not make this type of mistake in the commonly used measures of objects. Measure of the collection is not a collection.
...
\begin{frame}[allowframebreaks,label=equal_entropy]{Argument from Entropy and Order difference}
  Two different distributions can have the same Shannon entropy even if their internal structures differ significantly. There role of order measure of the order of information is currently  invastigated by philosophers (xxx Mann)
...
\begin{frame}[label=equal_entropy_cont]{Argument from Entropy and Order difference (cont.)}

This argument is flawed in at least two respects.

First, whats is equal here, is only the measure (entropy). Cf. a glass of water can carry the same amout of water as a bottle.    Shannon entropy captures the variability distribution but \textbf{does not capture the specific arrangement or structure of the data}.
    ...
    E.g. in retina this order may be preserved just by spatial encoding that is then encoded by time relations between spikes (Barlow hypothesis )
...
\begin{frame}[label=shannon_pipeline]{Shannon pipeline basic scheme}
    ...
    \subsection*{Original Shannon Communication Pipeline with Noisy Channel }
        \begin{center}
 ...
 \begin{frame}[label=shannon_noisy]{Shannon Communication Pipeline with Noisy Channel as Mechanistic Component}
                ...
                \vspace{0.5cm}
                \textbf{Properties of pipeline - a template}
                \begin{itemize}
                    \item \textbf{Task Function:} Reliable transmission or storage of information from point to point, fron Source to Destination. Intended a real world mechanism when applied.
                    \item \textbf{Order / Compositionalty  / systematicity:} First-In-First-Out queue; not compositional in normal sense, however FIFO is a kind of order preserving process.
...
\begin{frame}[label=semantic_shannon]{ Semantic properties of Shannon pipeline }
        ...
        \begin{tikzpicture}[remember picture, overlay]     
            \node at (4cm, 6cm) {\textbf{\textcolor{red}{accuracy condition }}};
             \node at (11cm, 6cm) {\textbf{\textcolor{red}{ information is grounded}}};
        \end{tikzpicture}
...
\begin{frame}[label=shannon_majority]{ Shannon model with redundancy encoding and majority vote decoder}
    ...
    This is the pipeline that sometimes can get very complex and thus not easy to identify.
    Consider the Scarantion  coins examples (\cite{scarantino_information_2015})
        The parts of whole pipelines that are using our logical inferences are not easy to identify but should be taken into account in the whole picture of cognitive pipeline 
...
\begin{frame}[label=no_gap, shrink] {The landscape after New Mechanists treatment}
    ...
    \textbf{There is no gap between kinds of information. }
    We deal with one kind of information (collection) - and many kinds of pipelines to discover and create.
